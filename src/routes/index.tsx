    ...
    <section className="border-b-2 border-outline bg-surface-container">
      <div className="max-w-7xl mx-auto px-4 py-12 md:py-20 grid lg:grid-cols-[1.2fr_1fr] gap-10 items-center">
        ...
        <div className="grid grid-cols-2 gap-3">
          {[
            { icon: "groups", label: "Co-working" },
            { icon: "rocket_launch", label: "Accelerators" },
            { icon: "school", label: "Universities" },
            { icon: "payments", label: "VCs" },
            { icon: "event", label: "Events" },
            { icon: "bolt", label: "Opportunities" },
          ].map((c, i) => (
            <div
              key={c.label}
              className={`p-4 rounded-2xl border-2 border-outline ${i % 3 === 0 ? "bg-accent text-accent-foreground" : "bg-surface"} shadow-brutal-sm`}
            >
...
function PersonalizedStrip({ userId }: { userId: string }) {
  ...
  const hasAny = data.events.length || data.opportunities.length || data.locations.length;
    ...
    <section className="border-b-2 border-outline bg-surface-container">
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="flex items-end justify-between gap-3 mb-4">
          <div>
            <span className="text-[10px] font-bold uppercase tracking-widest text-primary">{data.personalized ? "Tailored for you" : "Fresh in Berlin"}</span>
            <h2 className="font-display text-2xl md:text-3xl font-bold">For You</h2>
          </div>
          <Link to="/onboarding" className="text-xs font-semibold text-muted-foreground hover:text-primary uppercase tracking-widest">
            {data.personalized ? "Edit interests" : "Personalize"}
          </Link>
        </div>

        {data.events.length > 0 && (
          <FeedRow title="Events" icon="event" linkTo="/events">
            {data.events.map((e) => (
              <a key={e.id} href={e.url ?? "#"} target={e.url ? "_blank" : undefined} rel="noreferrer" className="block min-w-[260px] max-w-[260px] p-4 rounded-2xl bg-surface border-2 border-outline shadow-brutal-sm hover:shadow-brutal transition-shadow">
                <span className="text-[10px] font-bold uppercase tracking-widest text-primary">{new Date(e.starts_at).toLocaleDateString(undefined, { month: "short", day: "numeric" })}</span>
        ...
        {data.opportunities.length > 0 && (
          <FeedRow title="Opportunities" icon="bolt" linkTo="/opportunities">
            {data.opportunities.map((o) => (
              <a key={o.id} href={o.url ?? "#"} target={o.url ? "_blank" : undefined} rel="noreferrer" className="block min-w-[260px] max-w-[260px] p-4 rounded-2xl bg-surface border-2 border-outline shadow-brutal-sm">
        ...
        {data.locations.length > 0 && (
          <FeedRow title="Spaces & partners" icon="place" linkTo="/">
            {data.locations.map((l) => (
              <Link key={l.id} to="/location/$id" params={{ id: l.id }} className="block min-w-[260px] max-w-[260px] p-4 rounded-2xl bg-surface border-2 border-outline shadow-brutal-sm">
                <span className="text-[10px] font-bold uppercase tracking-widest text-primary">{CATEGORY_LABEL[l.category as LocationCategory] ?? l.category}</span>
...
function FeedRow({ title, icon, linkTo, children }: { title: string; icon: string; linkTo: string; children: React.ReactNode }) {
  return (
    <div className="mb-5 last:mb-0">
      <div className="flex items-center justify-between mb-2">
        ...
        <Link to={linkTo} className="text-[11px] font-semibold text-muted-foreground hover:text-primary uppercase tracking-widest">See all →</Link>
      </div>
      <div className="flex gap-3 overflow-x-auto no-scrollbar pb-1 -mx-1 px-1">{children}</div>
    </div>
  );
}
